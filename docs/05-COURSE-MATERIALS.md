...
1. **文件整理** → 从 `源文件/` 复制并重命名到 `src/data/courses/PX/LX/LX-*/`
2. **内容提取** → 运行 `python scripts/extract_content.py`
3. **数据开发** → 创建 `lessons/{课次}/lesson-data.js`
4. **无需修改组件** → 所有展示组件已自动加载数据
5. **测试页面** → 访问 `/lesson/PY2/L7/L7-2` 查看页面

### 目录结构说明

```
src/data/courses/PY1/
├── L1/                    # 单元1（用于放置整理后的源文件）
│   ├── L1-1/              # 课次1
│   │   ├── flashcard.png
│   │   ├── knowledge.png
...
① 文件整理：复制、重命名
    ↓
src/data/courses/PX/LX/LX-*/ (标准源文件)
    ↓
② 内容提取：extract_content.py
    ↓
...
| 原始文件名 | 标准命名 | 说明 |
|-----------|---------|------|
| `LX-X单词卡.png` / `单词卡.png` | `flashcard.png` | 英文单词卡 |
| `知识总结.png` / `LX-X知识总结.png` | `knowledge.png` | 知识点图 |
| `LX-X讲义.pdf` / `讲义.pdf` | `lecture.pdf` | 课堂讲义 |
| `LX-X课后习题.pdf` / `课后习题.pdf` / `课后练习题.pdf` | `exercise.pdf` | 练习题 |
| `LX-X课后习题解析.pdf` / `课后习题解析.pdf` / `课后练习题解析.pdf` | `solution.pdf` | 答案解析 |

### 版本选择原则

源文件中可能存在多个版本，按以下优先级选择：

1. **优先**：`新logo版` 或最新版本
2. **排除**：标注为"废用"、"旧版"、"迭代"的文件
3. **验证**：打开PDF确认内容完整性
...
```bash
# 1. 创建目标目录
mkdir -p src/data/courses/PY1/L1/L1-1

# 2. 复制并重命名文件
cp "源文件/level1/L1-1/L1-1单词卡.png" \
   "src/data/courses/PY1/L1/L1-1/flashcard.png"

cp "源文件/level1/L1-1/知识总结.png" \
   "src/data/courses/PY1/L1/L1-1/knowledge.png"

cp "源文件/level1/L1-1/L1-1讲义.pdf" \
   "src/data/courses/PY1/L1/L1-1/lecture.pdf"

cp "源文件/level1/L1-1/L1-1课后习题.pdf" \
   "src/data/courses/PY1/L1/L1-1/exercise.pdf"

cp "源文件/level1/L1-1/L1-1课后习题解析.pdf" \
   "src/data/courses/PY1/L1/L1-1/solution.pdf"
```
...
- `src/data/courses/PX/content.json` — 结构化 JSON 数据
- `src/data/courses/PX/OCR-CONTENT.md` — 可读 Markdown 文档
...
📍 当前位置：docs/05-COURSE-MATERIALS.md
   ...
   ① 了解文件整理规则（本节上文）
        ↓
📁 整理源文件到 courses/PX/LX/LX-*/
        ↓
   ② 运行内容提取脚本
        ↓
🐍 python scripts/extract_content.py
        ↓
📄 src/data/courses/PX/OCR-CONTENT.md
        ↓
   ③ 查看OCR提取的原始内容（参考材料）
        ↓
📄 src/data/courses/PX/GUIDE.md
        ↓
   ④ 了解内容筛选规则和校对步骤
        ↓
...
| 文件 | 说明 | 用途 |
|------|------|------|
| [docs/05-COURSE-MATERIALS.md](./05-COURSE-MATERIALS.md) | 本文档 | 完整流程、数据结构模板、开发规范 |
| [源文件/](../../源文件/) | 原始资料 | 下载的初始课程文件 |
| [src/data/courses/PX/LX/LX-*/](../src/data/courses/PY1/L1/L1-1/) | 标准源文件 | 整理后的原始文件（标准命名）|
| [scripts/extract_content.py](../scripts/extract_content.py) | 提取脚本 | OCR识别 + PDF文字提取 |
| [src/data/courses/PX/content.json](../src/data/courses/PY1/content.json) | 提取结果 | 结构化 JSON 数据 |
| [src/data/courses/PX/OCR-CONTENT.md](../src/data/courses/PY1/OCR-CONTENT.md) | 参考文档 | 可读的 OCR 提取内容 |
| [src/data/courses/PX/GUIDE.md](../src/data/courses/PY1/GUIDE.md) | 开发指南 | 内容筛选规则、校对步骤 |
