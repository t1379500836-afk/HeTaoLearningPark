...
**组件**: [views/YCLZoneView.vue](../src/views/YCLZoneView.vue)

#### 功能概述

为核桃编程学生提供 YCL（青少年人工智能编程水平测试）备考专区，包含考试介绍、考试信息、模拟练习等功能。

#### 模块划分

| 模块 | 说明 | 状态 |
|------|------|------|
| YCL介绍 | 什么是YCL、主办方、含金量、科技特长生政策 | 🔴 待开发 |
| 考试信息 | 题型、分值、时长、设备要求、证件要求 | ✅ 数据已配置 |
| 考情分析 | 四级/五级/六级知识点分布、分值占比、考查趋势 | ✅ 已完成 |
| 练习题库 | 每个等级5套练习题（3基础+1进阶+1提升） | 🟡 结构已创建，题目待生成 |
| 做题功能 | 倒计时、答题、自动评分、知识点诊断 | 🔴 待开发 |
| 权限控制 | 按路由前缀控制可访问的等级 | 🔴 待开发 |
...
| 等级 | 知识点数 | 核心考点 | 数据文件 |
|------|---------|---------|---------|
| 四级 | 12个 | 输出命令、数学运算、字符串拼接、if-else、for循环 | `YCL/level4/exam-analysis.js` |
| 五级 | 10个 | 索引、while循环、随机数模块、break/continue | `YCL/level5/exam-analysis.js` |
| 六级 | 9个 | 排序算法(20-25分)、递归(30-40分)、Pygame、模拟算法 | `YCL/level6/exam-analysis.js` |
...
> 详细文档：[06-YCL专区文档](./06-YCL-ZONE.md)
...
| PY1/PY3内容 | 阶段内容待开发 |
| 用户进度 | 用户进度跟踪功能 |
| **YCL专区** | YCL介绍页、做题功能、权限控制（考情分析、考试信息、理论讲解已配置） |
