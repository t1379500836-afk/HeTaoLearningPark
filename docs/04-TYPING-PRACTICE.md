...
- [快捷键练习组件](#快捷键练习组件-shortcutpracticevue)
...
| **ShortcutPractice** | 电脑快捷键（Ctrl+C/V/X/A/Z/S） | 内置数据 | 快捷键操作练习 |
...
## 快捷键练习组件 (ShortcutPractice.vue)

### 组件位置

**文件**: [components/course/ShortcutPractice.vue](../src/components/course/ShortcutPractice.vue)

### 功能说明

快捷键练习组件帮助小朋友学会常用的电脑快捷键，通过实际操作练习掌握复制、粘贴、剪切、全选、撤销、保存等基础操作。

### 练习内容

| 快捷键 | 名称 | 难度 | 练习方式 |
|--------|------|------|---------|
| **Ctrl+C** | 复制 | 简单 | 选中文本后按快捷键 |
| **Ctrl+V** | 粘贴 | 简单 | 在输入框中粘贴内容 |
| **Ctrl+X** | 剪切 | 中等 | 选中文本后按快捷键 |
| **Ctrl+A** | 全选 | 简单 | 在输入框中全选所有文字 |
| **Ctrl+Z** | 撤销 | 中等 | 先删除文字再撤销恢复 |
| **Ctrl+S** | 保存 | 困难 | 模拟保存（不实际保存文件） |

### 虚拟键盘高亮

- **组合键显示**: 同时高亮显示需要按的两个键（如 Ctrl 和 C）
- **脉冲动画**: 高亮按键带有呼吸动画效果
- **按键盘听**: 实时显示用户按下的键
...
- **快捷键卡片**: 显示当前练习的快捷键图标、名称和说明
- **组合键显示**: 橙色渐变大按键展示需要按的组合键
- **练习区域**:
  - 带文本输入：提供文本供选中/编辑操作
  - 无文本输入：显示提示区域等待快捷键
- **进度条**: 显示当前练习进度
- **跳过按钮**: 允许跳过当前练习
- **儿童友好提示**: "先按住 Ctrl 键，再按第二个键"
...
| 功能 | 实现方式 |
|------|---------|
| 复制检测 | `@copy` 事件 + `window.getSelection()` |
| 粘贴检测 | `@paste` 事件 + `clipboardData.getData()` |
| 剪切检测 | `@cut` 事件 + `window.getSelection()` |
| 全选检测 | `selectionStart === 0 && selectionEnd === textLength` |
| 撤销检测 | 浏览器自动处理 + 直接通过 |
| 保存模拟 | `e.preventDefault()` 阻止页面保存 |
...
- **中文模式**: 古诗词、成语或混合内容练习
- **快捷键模式**: 6个常用快捷键逐步练习（复制、粘贴、剪切、全选、撤销、保存）
