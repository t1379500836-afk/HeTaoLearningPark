...
### lessonData.meta.title 命名规则

课次标题应该使用**故事化名称**，避免使用技术性描述作为标题。

**规则**:
1. 课次标题(lessonMeta.title)应该采用故事化、趣味化的名称
2. README.md中模块名包含的技术性前缀（如"split命令"、"字典结构"等）不作为课次title
3. 只保留故事化部分作为课次标题

**示例对比**:

| README模块名 | 错误的title | 正确的title |
|-------------|------------|------------|
...
| 模块11：数据类型龙王传说 | 数据类型龙王传说 | 龙王传说 |
| 模块15：温故知新鸿门宴 | 温故知新鸿门宴 | 鸿门宴 |
| 模块19：循环嵌套世界遗忘我 | 循环嵌套 | 循环嵌套世界遗忘我 |
| 模块21：枚举法重生之我要好评 | 枚举法入门 | 枚举法重生之我要好评 |
| 模块23：枚举解配方美食大作战 | 枚举解配方美食大作战 | 美食大作战 |

**注意事项**:
- 技术性内容应该放在 subtitle（副标题）中，而不是 title（标题）中
- title 应该简洁、有趣、吸引小学生
- subtitle 可以包含学习目标和技术要点
